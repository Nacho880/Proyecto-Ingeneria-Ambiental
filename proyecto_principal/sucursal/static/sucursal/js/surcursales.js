...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const editSucursalModal = document.getElementById('editSucursalModal');
  const deleteSucursalModal = document.getElementById('deleteSucursalModal');
  const verDetallesSucursalModal = document.getElementById('verDetallesSucursalModal');
  
    // Mover los modales al final del body para evitar problemas de z-index
  if (editSucursalModal) {
    document.body.appendChild(editSucursalModal);
  }
  
  if (deleteSucursalModal) {
    document.body.appendChild(deleteSucursalModal);
  }
  
  if (verDetallesSucursalModal) {
    document.body.appendChild(verDetallesSucursalModal);
  }
  ...
  if (editSucursalModal) {
    editSucursalModal.addEventListener('show.bs.modal', function (event) {
      ...
      if (editTipo) editTipo.value = tipo || 'TIENDA';
      ...
      if (editForm) {
        editForm.action = `/sucursal/editar/${id}/`;
      }
    });
  ...
  if (deleteSucursalModal) {
    deleteSucursalModal.addEventListener('show.bs.modal', function (event) {
      const button = event.relatedTarget;
      ...
      document.getElementById('deleteSucursalForm').action = `/sucursal/eliminar/${id}/`;
    });
    // Limpiar mensaje de error al cerrar el modal
    deleteSucursalModal.addEventListener('hidden.bs.modal', function () {
      const alertDiv = deleteSucursalModal.querySelector('.modal-alert');
      if (alertDiv) {
        alertDiv.remove();
      }
    });
    ...
    editSucursalModal.addEventListener('hidden.bs.modal', function () {
      // Limpiar mensajes de error
      const alertDiv = editSucursalModal.querySelector('.modal-alert');
      if (alertDiv) alertDiv.remove();
      // Limpiar clases de error y mensajes de teléfono
      const inputs = editSucursalModal.querySelectorAll('.is-invalid');
      inputs.forEach(input => input.classList.remove('is-invalid'));
      const feedbacks = editSucursalModal.querySelectorAll('.invalid-feedback');
      feedbacks.forEach(fb => fb.remove());
      const telefonoErrors = editSucursalModal.querySelectorAll('.telefono-error');
  ...
  // Manejar el envío del formulario de eliminación con AJAX
  const deleteSucursalForm = document.getElementById('deleteSucursalForm');
  if (deleteSucursalForm) {
    
    deleteSucursalForm.addEventListener('submit', function (event) {
      
      event.preventDefault();
      event.stopPropagation();
      
      const formData = new FormData(this);
      const modal = document.getElementById('deleteSucursalModal');
      ...
      fetch('/sucursal/eliminar/' + formData.get('id') + '/', {
        method: 'POST',
        body: formData,
        headers: {
      ...
      .then(data => {
        if (data) {
          ...
          if (data.success) {
            ...
            const modal = bootstrap.Modal.getInstance(document.getElementById('deleteSucursalModal'));
            modal.hide();
            
            // Mostrar mensaje con botón deshacer
            mostrarMensajeConDeshacer(data.message, data.sucursal_id);
            
            // Actualizar la tabla dinámicamente
            actualizarTablaSucursales();
          } else {
            // Error
            mostrarMensajeEnModal('deleteSucursalModal', data.message, 'danger');
          }
  ...
  // Manejar el envío del formulario de editar sucursal con AJAX
  const editForm = document.getElementById('editSucursalForm');
  if (editForm) {
      ...
      .then(data => {
        if (data) {
          ...
          if (data.success) {
            ...
            const modal = bootstrap.Modal.getInstance(document.getElementById('editSucursalModal'));
            modal.hide();
            ...
            // Error
            mostrarMensajeEnModal('editSucursalModal', data.message, 'danger');
          }
  ...
  // Manejar el envío del formulario de agregar sucursal con AJAX
  const addSucursalForm = document.getElementById('addSucursalModal').querySelector('form');
  
  if (addSucursalForm) {
    
    addSucursalForm.addEventListener('submit', function (event) {
      ...
      if (telefono.length < minLength) {
        let alertDiv = addSucursalModal.querySelector('.modal-alert');
        if (!alertDiv) {
          alertDiv = document.createElement('div');
          alertDiv.className = 'modal-alert alert alert-danger mt-3 text-center';
          const modalFooter = addSucursalModal.querySelector('.modal-footer');
      ...
      .then(data => {
        if (data) {
          if (data.success) {
            const modal = bootstrap.Modal.getInstance(document.getElementById('addSucursalModal'));
            modal.hide();
            mostrarMensajeEnPagina(data.message, 'success');
            actualizarTablaSucursales();
          } else {
  ...
  const addModal = document.getElementById('addSucursalModal');
  if (addModal) {
  ...
  const editModal = document.getElementById('editSucursalModal');
  if (editModal) {
...
// Validación y limpieza para el modal de agregar sucursal
...
if (addSucursalModal && addSucursalForm) {
  // Limpiar formulario y mensajes al cerrar el modal
  addSucursalModal.addEventListener('hidden.bs.modal', function () {
    addSucursalForm.reset();
    // Limpiar mensajes de error
    const alertDiv = addSucursalModal.querySelector('.modal-alert');
    if (alertDiv) alertDiv.remove();
    // Limpiar clases de error y mensajes de teléfono
    const inputs = addSucursalModal.querySelectorAll('.is-invalid');
    inputs.forEach(input => input.classList.remove('is-invalid'));
    const feedbacks = addSucursalModal.querySelectorAll('.invalid-feedback');
    feedbacks.forEach(fb => fb.remove());
    const telefonoErrors = addSucursalModal.querySelectorAll('.telefono-error');
    telefonoErrors.forEach(error => error.remove());
  });
...
function mostrarMensajeConDeshacer(mensaje, sucursalId) {
      ...
      <div style="flex: 1;">
        ...
        <div style="text-align: center;">
          <button id="btnDeshacerEliminacion" 
                  onclick="restaurarSucursal(${sucursalId}, this)" 
...
// Función para restaurar sucursal
function restaurarSucursal(sucursalId, button) {
  ...
  fetch(`/sucursal/restaurar/${sucursalId}/`, {
    method: 'POST',
    headers: {
  ...
  .then(data => {
    if (data.success) {
      ...
      // Mostrar error
      alert('Error al restaurar la sucursal: ' + data.message);
...
// Función para actualizar la tabla de sucursales dinámicamente
function actualizarTablaSucursales() {
  ...
  // Si no existe el contenedor de la tabla, buscar el div con el mensaje "No hay sucursales"
  if (!tableWrapper) {
    // Buscar el div que contiene el mensaje de "No hay sucursales registradas"
